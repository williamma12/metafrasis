...
class TestAutogradIntegration:
    ...
    def test_gradcheck(self):
        ...
        from mps_ctc.ctc_loss import CTCLossFunctionNative

        # Use small sizes for gradcheck (it's slow)
        T, B, C, S = 8, 2, 5, 3
        torch.manual_seed(42)

        log_probs = torch.randn(T, B, C, device='mps', dtype=torch.float64).log_softmax(2)
        log_probs = log_probs.requires_grad_(True)
        targets = torch.randint(1, C, (B, S), device='mps').int()
